   ...
   ACCEPT DAY  PROMPT 'Day from to monitor (DD/MM/RRRR) DEF TODAY -> ' 
   ...
   and (sqls.SNAP_ID ,sqls.instance_number,Sqls.con_id ) IN (SELECT distinct SNAP_ID,instance_number,con_id from DBA_HIST_SNAPSHOT where end_interval_time BETWEEN to_date(to_char(cast(NVL('&DAY',SYSDATE) as date),'DD/MM/YYYY' ),'dd/mm/rrrr') 
